...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=[],
        help="Path prefix to exclude from scans (relative to root).",
    )
    parser.add_argument(
    ...
    for prefix in args.exclude:
        candidate = prefix if prefix.is_absolute() else (args.root / prefix)
        exclude_paths.append(candidate.resolve())

    # Find all Python files
    python_files = [
        path for path in args.root.rglob("*.py") if not should_exclude(path, exclude_paths)
    ]
