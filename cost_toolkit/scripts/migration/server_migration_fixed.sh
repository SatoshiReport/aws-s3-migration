...
echo "🚀 EBS to S3 Migration Script (Fixed)"
echo "====================================="
...
# Function to sync directory to S3 (without sudo for aws command)
sync_dir() {
    local source="$1"
    local s3_path="$2"
    ...
    if [ -d "$source" ] && [ "$(sudo ls -A $source 2>/dev/null)" ]; then
        ...
        temp_dir="/tmp/sync_$(basename $source)"
        sudo rm -rf "$temp_dir"
        sudo cp -r "$source" "$temp_dir"
        sudo chown -R ubuntu:ubuntu "$temp_dir"
        
        # Sync from temp directory (as ubuntu user with AWS credentials)
        aws s3 sync "$temp_dir" "s3://aws-user-files-backup-london/$s3_path/" \
            --region eu-west-2 \
