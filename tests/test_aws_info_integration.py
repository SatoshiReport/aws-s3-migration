...
def test_main_calls_get_aws_identity(mock_aws_info_context):
    ...
    with mock_aws_info_context.with_buckets([]):
        ...
        mock_aws_info_context.identity_mock.assert_called_once()


def test_main_calls_list_s3_buckets(mock_aws_info_context):
    ...
    with mock_aws_info_context.with_buckets(["bucket-1", "bucket-2"]):
        ...
        mock_aws_info_context.buckets_mock.assert_called_once()


def test_main_full_output_format_no_buckets(mock_aws_info_context):
    ...
    # Override default identity for this test
    ...
    with mock_aws_info_context.with_buckets([]):
        ...
        assert_equal(mock_aws_info_context.print_mock.call_count, 4)
        calls = [call[0][0] for call in mock_aws_info_context.print_mock.call_args_list]
...
def test_main_full_output_format_with_buckets(mock_aws_info_context):
    ...
    with mock_aws_info_context.with_buckets(["app-data", "logs", "backups"]):
        aws_info.main()
        ...
        assert_equal(mock_aws_info_context.print_mock.call_count, 7)
        calls = [call[0][0] for call in mock_aws_info_context.print_mock.call_args_list]
...
def test_main_bucket_formatting_has_proper_indentation(mock_aws_info_context):
    """Test that bucket names are printed with proper indentation (two spaces and dash)."""
    with mock_aws_info_context.with_buckets(["test-bucket"]):
        aws_info.main()
...
def test_main_prints_blank_line_between_identity_and_buckets(mock_aws_info_context):
...
def test_main_entry_point(mock_aws_info_context):
