...
def test_signal_handler_sets_interrupted_flags(migrator, mock_dependencies):
...
def test_run_from_glacier_restore_phase(migrator, mock_dependencies):
...
def test_run_from_glacier_wait_phase(migrator, mock_dependencies):
...
def test_run_from_syncing_phase(migrator, mock_dependencies):
...
def test_run_calls_drive_checker(migrator, mock_dependencies):
...
def test_run_prints_completion_message(migrator, mock_dependencies, capsys):
...
def test_phase_transitions_complete_workflow(migrator, mock_dependencies):
